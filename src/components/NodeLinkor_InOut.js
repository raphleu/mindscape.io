...
class NodeLinkor extends React.Component {
  constructor(props) {
    super(props);
  ...
  render() {
    ...
    const { getVect, user, path_press, out_defs, in_press, node, out_press, in_defs } = this.props;

    return (
      <div className='nodeLinkor' style={{
        ...
        <div className='out-defs linklist'>
        {
          out_defs.forEach(link => (
            <DefSummary_InOut key={'define-'+link.properties.id} getVect={getVect} user={user} path_press={path_press} node={node} def={link} />
          ))
        }
        </div>
        ---
        <div className='in-press linklist'>
        {
          in_press.forEach(link => (
            <PresSummary_InOut key={'present-'+link.properties.id} getVect={getVect} user={user} path_press={path_press} node={node} pres={link} />
          ))
        }
        </div>
...
export const NodeLinkor_InOut = connect((state, ownProps) => {
